...
export function InstallationModal({ project, isOpen, onClose }: InstallationModalProps) {
  ...
  // Create slides based on the layout
  const createSlides = () => {
    const slides = []
    let imageIndex = 0

    if (project.slidesLayout && project.images) {
      // Use custom layout
      for (let i = 0; i < project.slidesLayout.length; i++) {
        ...
        // Special naming for specific projects
      ...
      // Default: 1 photo per slide for installations
      ...
      // Generate placeholder slides based on photoCount and layout
      const photoCount = project.photoCount || 4

      if (project.slidesLayout) {
        let imageIndex = 0

        // Special handling for MSFTS project
        if (project.id === "msfts-ethiopia-skate-photos") {
          // First section: 5 landscape photos (16:9) - 2 per slide + 1 extra
          const landscapeSlides = Math.ceil(5 / 2) // 3 slides
          for (let i = 0; i < landscapeSlides; i++) {
            const photosInThisSlide = i === landscapeSlides - 1 ? 1 : 2 // Last slide has 1 photo
            const slideImages = []
          ...
          // Second section: 19 portrait photos (9:16) - 2 per slide + 1 extra
          const portraitSlides = Math.ceil(19 / 2) // 10 slides
          for (let i = 0; i < portraitSlides; i++) {
            const photosInThisSlide = i === portraitSlides - 1 ? 1 : 2 // Last slide has 1 photo
            const slideImages = []
          ...
          // Regular layout handling for other projects
          for (let i = 0; i < project.slidesLayout.length; i++) {
            const photosPerSlide = project.slidesLayout[i]
            const slideImages = []
    ...
    // Add Instagram link slide for graphic posters project
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-2xl w-[90vw] h-[90vh] flex overflow-hidden relative">
        ...
        <div className="w-[70%] relative overflow-hidden bg-neutral-50 flex items-center justify-center">
          {currentSlide?.type === "video" ? (
                ...
                <div className="space-y-4">
                  ...
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.073-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.441 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
            ...
            <div className="w-full h-full p-6">
              {currentSlide?.images.length === 1 ? (
                ...
                // 2 photos - side by side with proper spacing
                ...
                // Special bento layout for Bet Bota only
